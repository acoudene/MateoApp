...
namespace Mateo.Math.Managers
{
  ...
  {
    static readonly Dictionary<long, String> dic = new Dictionary<long, string>();
    static readonly Dictionary<long, String> logs = new Dictionary<long, string>();
    ...
    /// <returns></returns>
    public static string IntToFr(long nb)
    {
      ...
      for (int i = log; i > 1; i--)
      {
        ...
        {
          int master = (int)System.Math.Floor(System.Math.Pow(10, i)); // master = 1000
          long coeff = nb / master;           // coeff  = 3
          long reste = nb - (coeff * master);       // reste  = 25123
          return $"{(coeff > 1 ? IntToFr(coeff) : string.Empty)} {logs[i]}{(reste > 0 ? " " + IntToFr(reste) : string.Empty)}";
        }
      }
